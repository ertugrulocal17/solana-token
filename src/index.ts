...
async function main() {
  ...
  const reveicerTokenAccount = await createTokenAccount(
    connection,
    user,
    mint,
    receiver
  ...
  await transferTokens(
    ...
    reveicerTokenAccount.address,
    user.publicKey,
    50,
    mint
  );
  await burnTokens(connection, user, tokenAccount.address, mint, user, 50);

  // MAKE SURE YOU REPLACE THIS ADDRESS WITH YOURS!
  const MINT_ADDRESS = "6mnSmEcYwmLkGnxswG6vv2Z7xzukRsyBNNbgFH18pcox";
  ...
  await createTokenMetadata(
    ...
    user,
    "Ertugrul", // Token name - REPLACE THIS WITH YOURS
    "ERT", // Token symbol - REPLACE THIS WITH YOURS
